...
export async function do_queue_time(org, time) {
    try {
        ...
        queue.forEach((x) => outputGit(`${x.id} ${GRAY}│${NORMAL_COLOR} ${alignRight(x.r, 12)} ${GRAY}│${NORMAL_COLOR} ${alignLeft(x.e, 12)} ${GRAY}│${NORMAL_COLOR} ${alignLeft(x.s, 7)} ${GRAY}│${NORMAL_COLOR} ${new Date(x.q).toLocaleString()}`));
    }
    catch (e) {
        throw e;
    }
...
function histogram(data, min, max, numBins = Math.ceil(Math.sqrt(data.length))) {
    ...
    data.forEach((x) => {
        ...
        if (!(0 <= i && i < numBins))
            console.log(i, x.d, min, resolution);
        result[i]++;
    });
...
export async function queue(organizationId) {
    try {
        ...
        const varColumn = Math.min(size[0] - 41 - "─┼──┼──┼──┼──┼─".length - "> [_] ".length, longestEvent + 1);
        ...
        Object.values(buckets).forEach((p) => {
            ...
            buckets.forEach((bw) => {
                bw.forEach((b) => {
                    ...
                    options.push({
                        long: latest.i,
                        text: `${alignRight(p.e, varColumn)} ${GRAY}│${NORMAL_COLOR} ${status} ${GRAY}│${NORMAL_COLOR} ${alignRight(b.length === 1 ? "" : b.length.toString(), 5)} ${GRAY}│${NORMAL_COLOR} ${WEEKDAYS[latest.l.getDay()]} ${GRAY}│${NORMAL_COLOR} ${responseTime} ${GRAY}│${NORMAL_COLOR} ${time}`,
