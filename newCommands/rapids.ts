...
const FIXED_COLUMNS_WIDTH =
  STATUS_WIDTH + COUNT_WIDTH + DAY_WIDTH + RESP_TIMES_WIDTH + LATEST_WIDTH;
      ...
      async () => {
        ...
        const tableHeader = (() => {
          ...
          parsed.is.forEach((p, i) => {
            if (
              buckets[parsed.es[i] + parsed.ss[i] + parsed.hs[i]] === undefined
            )
          ...
          Object.values(buckets).forEach((p) => {
            ...
            buckets.forEach((bw) => {
              bw.forEach((b) => {
                ...
                const st = p.s.split("/");
                const interest =
                  st.length === 1 || +st[2] + +st[1] > 0
                    ? Interest.INTERESTING
                    : b.length === 1
                    ? Interest.MODERATELY_INTERESTING
                    : Interest.MILDLY_INTERESTING;
                const text = (() => {
                  if (![undefined, "x"].includes(getArgs()[0])) return ``;
                  ...
                  const time =
                    timeDiff < 4 * HOURS
                      ? Str.alignRight(
                          Str.withUnit(timeDiff, UnitType.Duration),
                          5
                        ) + " ago"
