...
export async function do_renameOrganization(organizationId, displayName) {
    if (isDryrun()) {
        output("DRYRUN: Would rename organization");
        return;
    }
    try {
        const reply = await sshReq(`organization-rename`, displayName, `--organizationId`, organizationId.toString());
    }
    catch (e) {
        throw e;
    }
...
export function generateOrgName() {
    if (process.env["MERRYMAKE_NAME_LENGTH"] !== undefined &&
        ...
        return (base +
            generateString(+process.env["MERRYMAKE_NAME_LENGTH"] - base.length, lowercase + digits));
    }
    else
...
function do_join_email_wait(org) {
...
async function do_join_more(org, emails) {
    try {
        return choice([], async () => {
            return {
...
async function join() {
    try {
        const name = await shortText("Organization to join", "Name of the organization you wish to request access to.", "Acme Corp").then();
        ...
        return choice([
            ...
            if (orgs.length > 0) {
                options.push({
                    ...
                    action: () => checkout_org(orgs[0].name, new OrganizationId(orgs[0].id)),
                });
            }
            if (orgs.length > 1) {
                options.push({
