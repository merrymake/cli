...
async function queue_event(id, river) {
    try {
        const res = JSON.parse(await sshReq(`rapids-inspect`, `\\"${id}\\"`, `--river`, river));
...
function queue_id(id) {
    ...
    return choice("Which event would you like to inspect?\n" + tableHeader, cache_queue
        .filter((x) => x.id === id)
        .map((x) => ({
        long: x.r,
        text: `${alignRight(x.r, 12)} │ ${alignLeft(x.e, 12)} │ ${alignLeft(x.s, 7)} │ ${new Date(x.q).toLocaleString()}`,
...
async function queue_time(org) {
    try {
        let d = new Date(await shortText("Time", "Displays events _around_ specified time.", "1995-12-17T03:24:00")).getTime();
        while (isNaN(d)) {
            outputGit("Invalid date, please try again.");
            d = new Date(await shortText("Time", "Displays events _around_ specified time.", "1995-12-17T03:24:00")).getTime();
        }
...
export async function queue(organizationId) {
    try {
        ...
        options.push(...cache_queue.map((x) => ({
            long: x.id,
            text: `${x.id} │ ${alignLeft(x.r + "/" + x.e, 19)} │ ${alignLeft(x.s.substring(0, 4), 4)} │ ${new Date(x.q).toLocaleString()}`,
            action: () => {
                if (getArgs().length === 0)
                    initializeArgs([x.r]);
        ...
        return await choice("Which event would you like to inspect?" + tableHeader, options, {
