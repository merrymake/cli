...
export async function do_spending(org: string) {
  try {
    ...
    rows.forEach((x) => {
      ...
      outputGit(
        `${alignLeft(group, 11)} ${Str.FG_GRAY}│${Str.FG_DEFAULT} ${alignLeft(
          service,
          11
        )} ${Str.FG_GRAY}│${Str.FG_DEFAULT} ${alignLeft(hook, 20)} ${
          Str.FG_GRAY
        }│${Str.FG_DEFAULT} ${alignRight(
          "" + count_str + " " + count_unit,
          7
        )} ${Str.FG_GRAY}│${Str.FG_DEFAULT} ${alignRight(
          "" + time_str + " " + time_unit,
          7
        )} ${Str.FG_GRAY}│${Str.FG_DEFAULT} € ${alignRight(x.cost_eur, 7)}`
      );
    });
  } catch (e) {
    throw e;
